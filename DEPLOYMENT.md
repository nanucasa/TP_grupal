...
El modelo campeón se selecciona automáticamente a partir de los runs registrados en MLflow (tracking remoto en DagsHub), usando Dagster (sensor `champion_sensor` + assets dedicados), que:

- Toma el experimento `telco_churn_tune_xgb`.
- Considera los runs que tienen registrada la métrica `f1`.
- Ordena los resultados por `metrics.f1` de mayor a menor.
- Elige el mejor como **champion**, actualiza `artifacts/champion_metadata.json` y asigna el alias `champion` en el Model Registry de MLflow.
...
champion_path = Path("artifacts/champion_metadata.json")
with open(champion_path, "r", encoding="utf-8") as f:
    champion = json.load(f)

# Versión simple: cargar el modelo final entrenado
# En este TP el modelo final se guarda como models/model.joblib
...
El servicio puede actualizarse leyendo periódicamente artifacts/champion_metadata.json o exponiendo un endpoint interno para recargar el modelo cuando cambie el champion.
...
dvc pull
dvc repro train
Esto vuelve a correr el pipeline de preparación y entrenamiento.
...
Con Dagster en ejecución (dagster dev), el sensor champion_sensor:
consulta el experimento telco_churn_tune_xgb en MLflow,
ordena los runs por metrics.f1, si encuentra un F1 superior al actual, actualiza artifacts/champion_metadata.json y el alias champion en el Model Registry.

**4- Actualización de servicios**
API REST: El servicio recarga el modelo (por lectura de artifacts/champion_metadata.json o por reinicio controlado del contenedor).
...
lógica para cargar el champion desde MLflow / artifacts/champion_metadata.json.
